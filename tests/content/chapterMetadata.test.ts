...
describe("chapter metadata", () => {
  it("loads chapter metadata records from content/manual/chapters", async () => {
    ...
    expect(chapters.map((entry) => entry.chapterId)).toEqual([
      ...
      "chapter-08-bridgehead-post",
      "chapter-09-river-watch-post",
      "chapter-10-marsh-outpost",
      "chapter-11-reed-ferry",
      "chapter-template",
    ]);
    ...
    expect(chapters.find((entry) => entry.chapterId === "chapter-08-bridgehead-post")?.maps).toEqual([
      "bridge-road",
      "bridgehead-post",
    ]);
    expect(chapters.find((entry) => entry.chapterId === "chapter-09-river-watch-post")?.maps).toEqual([
      "east-bank-road",
      "river-watch-post",
    ]);
    expect(chapters.find((entry) => entry.chapterId === "chapter-10-marsh-outpost")?.maps).toEqual([
      "marsh-road",
      "marsh-outpost",
    ]);
    expect(chapters.find((entry) => entry.chapterId === "chapter-11-reed-ferry")?.maps).toEqual([
      "reed-ferry-road",
      "reed-ferry-camp",
    ]);
  });
