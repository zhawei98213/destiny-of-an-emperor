...
| 商店商品与价格一致性 / Shop Inventory And Price Parity | Lou Sang 供给点交互骨架 | `lou-sang-shop-event` + `shops[starter-shop]` + `shopStates` | 当前商店表来自既有 curated 数据 | `shop-counter-opens-shop-state` 回归 case | 有偏差 / Diverged | 商店触发与 `shopStates.visited` 已接通，但商品列表和价格仍非真实片段数据，且暂无独立商店 UI | 导入真实商店表，并决定是否扩展专用 shop UI |
