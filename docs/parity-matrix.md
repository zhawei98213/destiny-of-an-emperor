...
| 维度 | 复刻对象 | 当前系统承载位置 | 数据来源 | 验证方法 | 当前状态 | 偏差说明 | 下一步动作 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| NPC 位置、朝向一致性 / NPC Position And Facing Parity | `inner-ford-clerk`、`inner-ford-sentry`、`inner-ford-quartermaster`、`inner-ford-boatman` | `content/manual/world.content.json` -> `maps[].npcs` | `content/source/maps/inner-ford.source.json` + `npc-placement-import` | `npm run npc-placement-import -- --chapter chapter-12-inner-ford` + 运行时站位核对 | 进行中 / In Progress | 位置和朝向已导入并继续复用 clerk / guard / merchant / guide 模式；仍缺原始参考图逐点锁定 | 后续补 `inner-ford-road` 与 `inner-ford-camp` 的 NPC 参考帧并锁定坐标 |
| 事件触发条件一致性 / Event Trigger Condition Parity | `inner-ford-clerk-trigger`、`inner-ford-sentry-trigger`、`inner-ford-gate-trigger`、`inner-ford-shop-trigger`、`inner-ford-cache-trigger` | `content/manual/world.content.json` -> `maps[inner-ford-road/inner-ford-camp].triggers` | `content/source/maps/inner-ford.source.json` + `content/source/text/chapter-12-inner-ford.source.json` | `inner-ford-clerk-issues-orders`、`inner-ford-sentry-opens-gate`、`inner-ford-gate-warps`、`inner-ford-shop-opens`、`inner-ford-cache-first-open` | 已验证 / Verified | 第十二章继续复用 clerk -> sentry -> gate -> shop -> chest 模式，没有为新区域引入 scene 特判；本章新增模式只是“第二道河渡检查点”，不是新 DSL 需求 | 后续只需按真实资料微调对白与奖励，不需要增加新流程 |
| Flag 变化一致性 / Flag Mutation Parity | `inner-ford-orders-issued`、`inner-ford-gate-open`、`inner-ford-cache-opened` | `content/manual/story.content.json` -> `events[].steps`；`content/manual/world.content.json` -> `flags` | `content/source/text/chapter-12-inner-ford.source.json` + curated event rows | regression smoke + save/load 路径复测 | 进行中 / In Progress | 当前“文吏发军文 -> 守兵放桥 -> 渡桥切入内渡营 -> 领取补给”链路已接通；剩余差异是原始资料尚未核对放桥时机与木箱奖励 | 后续按原始章节资料微调放行和奖励时机 |
| 地图切换与出生点一致性 / Map Transition And Spawn Parity | `reed-ferry-camp -> inner-ford-road -> inner-ford-camp` 与回退链路 | `reed-ferry-east-crossing-event`、`inner-ford-gate-event`、`portals`、`spawnPoints` | `content/source/maps/reed-ferry.source.json` + `content/source/maps/inner-ford.source.json` | chapter-12 warp regression cases + 运行时往返切图 | 进行中 / In Progress | 前进链路有 regression 覆盖，完整回退链路仍以手工验证为主 | 后续补一条完整往返的 golden case |
| 宝箱/物品获取一致性 / Chest And Item Gain Parity | 内渡补给箱一次性领取 `bandage` + `travel-ration` | `inner-ford-cache-event` + `inner-ford-cache-trigger` | 当前奖励为 curated 版本，尚未接入原始物品记录 | `inner-ford-cache-first-open` 回归 case + 读档复测 | 有偏差 / Diverged | 一次性逻辑已验证，但奖励内容还没有原始资料核对 | 核对原始木箱奖励，再决定是否替换当前组合 |
| 商店商品与价格一致性 / Shop Inventory And Price Parity | `inner-ford-shop` 内渡粮官补给 | `inner-ford-shop-event` + `shops[inner-ford-shop]` + `shopOverlay` + `shopStates` | 当前商店表来自 curated chapter-12 版本 | `inner-ford-shop-opens` 回归 case + 商店 overlay 核对 | 进行中 / In Progress | 第十二章继续复用现有商店模式，货表已可显示并回归，但仍非真实资料最终版 | 后续导入真实商店表，并继续复用现有 UI 校验路径 |
| 战斗敌群与奖励一致性 / Battle Group And Reward Parity | `inner-ford-road-battle-trigger` -> `inner-ford-road-raiders` -> `highland-outlaws` | `content/manual/world.content.json` -> `encounterTables` + `content/generated/battle.content.json` + `game/src/battle/battleRuntime.ts` | `content/source/data/demo-game-data.source.json` | `inner-ford-road-battle-roundtrip` + `docs/chapters/chapter-12-inner-ford-battle-parity-notes.md` + `reports/battle-parity/latest/summary.md` | 进行中 / In Progress | 第十二章继续复用共享乱兵战，以验证第二道河渡检查点也能维持标准 battle slice；运行时闭环已打通，但 `highland-outlaws` 仍是当前 curated 数据，不是最终原始敌群表 | 后续若拿到真实敌群表，再决定是否拆出 chapter-12 专属 battle group 并复跑 battle parity |
